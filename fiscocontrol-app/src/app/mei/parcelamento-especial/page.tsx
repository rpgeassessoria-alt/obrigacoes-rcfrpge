...
export default function ParcelamentoEspecialMeiPage() {
    ...
    const handleConsultar = async () => {
        ...
        try {
             // Endpoint para o PARCMEI Especial
             const res = await axios.get(`/api/integra-contador/mei/parcelamento-especial?cnpj=${cnpj.replace(/\D/g, '')}`);
             setParcelData(res.data);
        } catch (err: any) {
             setError(err.response?.data?.message || `Erro ao consultar Parcelamento Especial no Serpro.`);
        } finally {
             setLoading(false);
        }
    }
    ...
    return (
        <div className="page-content">
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                Home / <span style={{ color: '#3b82f6' }}>VERI MEI</span> / Parcelamento Especial
            </div>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Parcelamento Especial MEI</h1>
                    <p className="page-sub">Consulta Serpro para parcelamentos com condições diferenciadas</p>
                </div>
            </div>
            ...
            {parcelData ? (
                 <div className="card">
                     <div className="card-title" style={{marginBottom: 20}}>Extrato do Parcelamento Especial</div>
