...
export default function ParcelamentoPertMeiPage() {
    ...
    const handleConsultar = async () => {
        ...
        try {
             // Endpoint para o PERT MEI
             const res = await axios.get(`/api/integra-contador/mei/parcelamento-pert?cnpj=${cnpj.replace(/\D/g, '')}`);
             setParcelData(res.data);
        } catch (err: any) {
             setError(err.response?.data?.message || `Erro ao consultar PERT MEI no Serpro.`);
        } finally {
             setLoading(false);
        }
    }
    ...
    return (
        <div className="page-content">
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                Home / <span style={{ color: '#3b82f6' }}>VERI MEI</span> / Parcelamento PERT MEI
            </div>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Parcelamento PERT MEI</h1>
                    <p className="page-sub">Consulta Serpro (Programa Especial de Regularização Tributária)</p>
                </div>
            </div>
            ...
            {parcelData ? (
                 <div className="card">
                     <div className="card-title" style={{marginBottom: 20}}>Extrato do Parcelamento PERT</div>
