...
import { PertMeiService } from '../../../../../lib/integra-contador/mei/pertmei';
...
export async function GET(request: Request) {
  ...
  try {
    const data = await PertMeiService.consultarExtrato(cnpj);
...
export async function POST(request: Request) {
  try {
    ...
    const data = await PertMeiService.emitirParcela(cnpj, numeroParcela);
    return NextResponse.json(data);
  } catch (error: any) {
    return NextResponse.json(error.response?.data || { error: error.message }, { status: error.response?.status || 500 });
  }
