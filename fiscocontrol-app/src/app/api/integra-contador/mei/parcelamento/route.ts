...
import { ParcMeiService } from '../../../../../lib/integra-contador/mei/parcmei';
...
export async function GET(request: Request) {
  ...
  try {
    const data = await ParcMeiService.consultarExtrato(cnpj);
...
export async function POST(request: Request) {
  try {
    ...
    const data = await ParcMeiService.emitirParcela(cnpj, numeroParcela);
    return NextResponse.json(data);
  } catch (error: any) {
    return NextResponse.json(error.response?.data || { error: error.message }, { status: error.response?.status || 500 });
  }
