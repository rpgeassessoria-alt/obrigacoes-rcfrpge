...
export default function CndRfbPgfnPage() {
    ...
    const handleConsultar = async () => {
        ...
        try {
             const res = await axios.get(`/api/integra-contador/certidoes/cnd-federal?cnpj=${cnpj.replace(/\D/g, '')}`);
             setCertData(res.data);
        } catch (err: any) {
             console.error("Erro na consulta CND", err);
             setError(err.response?.data?.message || "Erro ao consultar Certidão na Receita Federal (Serpro).");
        } finally {
             setLoading(false);
        }
    }
    ...
    return (
        <div className="page-content">
            <div className="page-header">
                <div>
                    <h1 className="page-title">CND RFB/PGFN</h1>
                    <p className="page-sub">Certidão Negativa de Débitos relativos a Créditos Tributários Federais e à Dívida Ativa da União</p>
                </div>
            </div>
            ...
            <div className="summary-cards">
                <div className="summary-card summary-card--regular">
                    <CheckCircle size={20} />
                    <div>
                        <span className="summary-num">28</span>
                ...
                <div className="summary-card summary-card--error">
                    ...
                    <div>
                        <span className="summary-num">10</span>
            ...
            <div className="card" style={{ marginBottom: 20 }}>
                <div className="card-title">Consultar Certidão Individual</div>
                <div style={{display: 'flex', gap: 10, alignItems: 'flex-end', marginTop: 15}}>
                     ...
                     <button className="btn-primary" onClick={handleConsultar} disabled={loading} style={{display: 'flex', alignItems: 'center', gap: 8}}>
                         {loading ? "Consultando..." : <><Search size={18} /> Emitir CND (Serpro)</>}
                     </button>
                </div>
            ...
            {certData ? (
                ...
                <div className="card">
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>
                        Utilize o campo acima para emitir uma nova certidão via Integra Contador.
                    </p>
                </div>
            )}
